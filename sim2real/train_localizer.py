...
from sim2real.shared.loaders import localizer_loader
...
run(
    ...
    config={
        ...
        'train_dataset': 'sim_plane_elastic_bkg',
        '{train_transform}': lambda: transform(image_aug=True, regen_tactile=True),
        'val_dataset': 'sim_plane_elastic_bkg',
        '{val_transform}': lambda: transform(image_aug=False, regen_tactile=True),
        '{data_loader}': lambda: localizer_loader('train', e['batches_per_epoch'], e['batch_size'], e['train_dataset']),
        '{val_loader}': lambda: localizer_loader('val', e['n_val_batches'], e['batch_size'], e['val_dataset']),
